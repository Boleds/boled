...
Exp=$(curl -sS http://galaxycode.co-id.id/vpn/ip | grep $MYIP | awk '{print $3}')
fi
export RED='\033[0;31m'
...
Exp2=$(curl -sS http://galaxycode.co-id.id/vpn/ip | grep $MYIP | awk '{print $3}')
d1=$(date -d "$Exp2" +%s)
d2=$(date -d "$today" +%s)
certificate=$(( (d1 - d2) / 86400 ))
Name2=$(curl -sS http://galaxycode.co-id.id/vpn/ip | grep $MYIP | awk '{print $2}')
